...
rain_2012<- lapply(1:length(annualRain),function(i) 
  get2012(annualRain[[i]][[1]]))
...
rain_data = data.frame(date = as.Date(ymd(paste(2012, 1:12, 1))), 
                       merged_data)
@
<<>>=
library(gstat)
...
v = vgmST("separable", space = vgm(1, "Exp", 75), 
          time = vgm(1, "Exp", 1.5), sill=0.6)
...
ggplot() + 
  geom_raster(aes(x, y, fill=rain), predicted) +
  facet_wrap(~fdate) +
  scale_fill_gradient(low = "steelblue",high = "red") 
@
<<fig=TRUE, echo=FALSE>>=
print(ggplot() + geom_raster(aes(x, y, fill=rain), predicted, alpha=0.5) + facet_wrap(~fdate) +
  scale_fill_gradient(low = "steelblue",high = "red") )
@
